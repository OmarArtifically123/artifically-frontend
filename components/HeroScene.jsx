...
function CameraRig({ reduceMotion, targetAspect = HERO_ASPECT, quality }) {
  ...
  useFrame(({ clock }) => {
    ...
    const horizontalDrift = reduceMotion ? 0 : Math.sin(BASE_CYCLE_FREQUENCY * cycleTime) * 0.05 * driftScale;
    const verticalBreath = reduceMotion ? 0 : Math.sin(BASE_CYCLE_FREQUENCY * cycleTime * 2) * 0.01 * driftScale;
...
export default function HeroScene({ width = 1280, height = 720 }) {
  ...
  const shouldRenderStats = process.env.NODE_ENV !== "production";
