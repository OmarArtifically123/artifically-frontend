...
const HeroSceneContent = lazy(async () => {
  const [{ Canvas, useFrame }, { Points, PointMaterial }] = await Promise.all([
  ...
  function HeroSceneCanvas({ width = 1280, height = 720 }) {
    return (
      <Canvas
        dpr={[1, 1.8]}
  ...
  return { default: HeroSceneCanvas };
});

export default function HeroScene(props) {
  return (
    <Suspense fallback={null}>
      <HeroSceneContent {...props} />
    </Suspense>
  );
}